# MorphEnterprise concept documentation

**Start here (canonical, English):** [en/00-overview.md](en/00-overview.md)

**Русская версия:** [ru/00-overview.md](ru/00-overview.md)

| Path | Language | Role |
|------|----------|------|
| [en/](en/) | English | Canonical specification (growing) |
| [ru/](ru/) | Russian | Full detail chapters 01–10 |
| [schemas/](schemas/) | neutral | YAML contracts |
| [lessons.md](lessons.md) | Russian | Dogfooding lessons |
| [../references/](../references/) | Russian | External research (Knowledge layer; non-normative) |
